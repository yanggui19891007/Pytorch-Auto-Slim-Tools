...
class Autoslim(object):
    ...
    def base_prunging(self, config):
        if not config['pruning_func'] in self.pruning_func:
            raise KeyError(
                "-[ERROR] {} pruning not supported.".format(config['pruning_func']))
    ...
    def _base_l1_pruning(self, config):
        return self.__base_lx_norm_pruning(config, norm='l1')

    def _base_l1_pruning(self, config):
        return self.__base_lx_norm_pruning(config, norm='l2')

    def __base_lx_norm_pruning(self, config, norm='l1'):
        prune_indexes = {}

        def _compute_lx_norm(m, norm):
            weight = m.weight.detach().cpu().numpy()
            if isinstance(m, nn.modules.conv._ConvTransposeMixin):
                if norm == 'l1':
                    Lx_norm = np.sum(np.abs(weight), axis=(0, 2, 3))
                else:
                    Lx_norm = np.sum(
                        np.sqrt(weight ** 2), axis=(0, 2, 3))
            else:
                if norm == 'l1':
                    Lx_norm = np.sum(np.abs(weight), axis=(1, 2, 3))
                else:
                    # 注：反卷积维数1对应输出维度
                    Lx_norm = np.sum(
                        np.sqrt(weight ** 2), axis=(1, 2, 3))
            return Lx_norm
        ...
        if config['global_pruning']:
            ...
            for i, m in enumerate(self.model_modules):
                if isinstance(m, nn.modules.conv._ConvNd):
                    Lx_norm = _compute_lx_norm(m, norm)
                    filter_record.append(Lx_norm.tolist())  # 记录每层卷积的lx_norm参数
            ...
            for i, m in enumerate(self.model_modules):
                if isinstance(m, nn.modules.conv._ConvNd):
                    ...
                    if isinstance(m, nn.modules.conv._ConvTransposeMixin):
                        Lx_norm = np.sum(np.abs(weight), axis=(
                            0, 2, 3))  # 注：反卷积维数1对应输出维度
                    else:
                        Lx_norm = np.sum(np.abs(weight), axis=(1, 2, 3))
                    num_pruned = min(int(max_ratio*len(Lx_norm)),
                                     len(Lx_norm[Lx_norm < thre]))  # 不能全部减去
                    # 删除低于阈值的卷积核
                    prune_index = np.argsort(Lx_norm)[:num_pruned].tolist()
                    prune_indexes[i] = prune_index
